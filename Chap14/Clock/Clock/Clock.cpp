...
void CMainWindow::OnTimer(UINT nTimerID)
{
	...
	// If seconds have changed, erase the second hand and redraw all the hands.
	if (nSecond != m_nPrevSecond)
	{
...
void CMainWindow::DrawClockFace(CDC* pDC)
{
	static CPoint point[12] = {
		...
		CPoint(255, 390),	// 1 o'clock
		CPoint(390, 255),	// 2 o'clock
	...
	for (int i = 0; i < 12; i++)
	{
		pDC->Rectangle(point[i].x - SQUARESIZE, point[i].y + SQUARESIZE, point[i].x + SQUARESIZE, point[i].y - SQUARESIZE);
	}
}
...
void CMainWindow::UpdateSystemMenu(CMenu* pMenu)
{
	...
	pMenu->GetMenuString(SC_MOVE, strMenuText, MF_BYCOMMAND);
	pMenu->ModifyMenu(SC_MOVE, MF_STRING | nState[i][2], SC_MOVE, strMenuText);

	pMenu->GetMenuString(SC_MOVE, strMenuText, MF_BYCOMMAND);
	pMenu->ModifyMenu(SC_MOVE, MF_STRING | nState[i][3], SC_MOVE, strMenuText);

	pMenu->GetMenuString(SC_MOVE, strMenuText, MF_BYCOMMAND);
	pMenu->ModifyMenu(SC_MOVE, MF_STRING | nState[i][4], SC_MOVE, strMenuText);
