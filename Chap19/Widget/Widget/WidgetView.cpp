...
BEGIN_MESSAGE_MAP(CWidgetView, CScrollView)
	ON_COMMAND(ID_EDIT_CUT, &CWidgetView::OnEditCut)
	ON_COMMAND(ID_EDIT_COPY, &CWidgetView::OnEditCopy)
	ON_COMMAND(ID_EDIT_PASTE, &CWidgetView::OnEditPaste)
	ON_COMMAND(ID_EDIT_DELETE, &CWidgetView::OnEditDelete)
	ON_UPDATE_COMMAND_UI(ID_EDIT_CUT, &CWidgetView::OnUpdateEditCut)
	ON_UPDATE_COMMAND_UI(ID_EDIT_COPY, &CWidgetView::OnUpdateEditCopy)
	ON_UPDATE_COMMAND_UI(ID_EDIT_PASTE, &CWidgetView::OnUpdateEditPaste)
	ON_UPDATE_COMMAND_UI(ID_EDIT_DELETE, &CWidgetView::OnUpdateEditDelete)
	ON_WM_SETFOCUS()
	ON_WM_KILLFOCUS()
	ON_WM_LBUTTONDOWN()
	ON_WM_CREATE()
...
CWidgetView::CWidgetView() noexcept
{
}
...
CWidgetDoc* CWidgetView::GetDocument() const // non-debug version is inline
{
...
DROPEFFECT CWidgetView::OnDragEnter(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point)
{
...
DROPEFFECT CWidgetView::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point)
{
...
void CWidgetView::OnEditPaste()
{
	...
	if (hData != NULL)
	{
		WIDGETINFO* pWidgetInfo = (WIDGETINFO*)::GlobalUnlock(hData);
		int x = pWidgetInfo->x;
		int y = pWidgetInfo->y;
		COLORREF color = pWidgetInfo->color;
		::GlobalUnlock(hData);
