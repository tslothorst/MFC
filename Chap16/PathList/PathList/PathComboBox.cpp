...
BOOL CPathComboBox::SetPath(LPCTSTR pszPath)
{
	if (m_bFirstCall)
	{
		...
		// Add icons respresenting the drives on the host system
		int nPos = 0;
		int nCount = 0;
		CString string = _T("?:\\");
	...
	// Find the item that corresponds to the drive specifier in the pszPath
	...
	// Delete the previously added folder item (if any)
...
void CPathComboBox::GetSubstring(int& nStart, CString& string, CString& result)
{
	result = _T("");
