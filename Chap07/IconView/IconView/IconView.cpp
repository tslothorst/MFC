...
CIconView IconViewApp;

BOOL CIconView::InitInstance()
{
...
CMainWindow::CMainWindow()
{
	CString strWndClass = AfxRegisterWndClass(
		0,
		IconViewApp.LoadStandardCursor(IDC_ARROW),
		(HBRUSH)(COLOR_3DFACE + 1),
		IconViewApp.LoadStandardIcon(IDI_WINLOGO)
	);
...
int CMainWindow::OnCreate(LPCREATESTRUCT lpcs)
{
	...
	m_wndGroupBox.Create(_T("Detail"), WS_CHILD | WS_VISIBLE | BS_GROUPBOX, CRect(m_cxChar * 2, m_cyChar, m_cxChar * 48, m_cyChar * 20), this, (UINT)-1);

	m_wndLabel.Create(_T("Icons"), WS_CHILD | WS_VISIBLE | SS_LEFT, CRect(m_cxChar * 50, m_cyChar, m_cxChar * 82, m_cyChar * 2), this);

	m_wndIconListBox.Create(WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_BORDER | LBS_NOTIFY | LBS_NOINTEGRALHEIGHT, CRect(m_cxChar * 50, m_cyChar * 2, m_cxChar * 82, m_cyChar * 20), this, IDC_LISTBOX);
...
void CMainWindow::OnDropFiles(HDROP hDropInfo)
{
	...
	if (nCount == 1) // One file at a time please
	{
		...
		// Extract the file's icons and add them to the listbox
...
int CIconListBox::AddIcon(HICON hIcon)
{
	...
	if ((nIndex != LB_ERR) && (nIndex != LBN_ERRSPACE))
	{
