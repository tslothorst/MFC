...
CMainWindow::CMainWindow()
{
	CString strWndClass = AfxRegisterWndClass(
		0,
		FontView.LoadStandardCursor(IDC_ARROW),
		(HBRUSH)(COLOR_3DFACE + 1),
		FontView.LoadStandardIcon(IDI_WINLOGO)
	);
	CreateEx(0, strWndClass, _T("FontView"), WS_OVERLAPPED | WS_SYSMENU | WS_CAPTION | WS_MINIMIZEBOX, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, NULL);
...
int CMainWindow::OnCreate(LPCREATESTRUCT lpcs)
{
	...
	m_wndCheckBox.Create(_T("Show TrueType fonts only"), WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, rect, this, IDC_CHECKBOX);

	rect.SetRect(m_cxChar * 2, m_cyChar * 21, m_cxChar * 66, m_cyChar * 25);
	m_wndGroupBox.Create(_T("Sample"), WS_CHILD | WS_VISIBLE | BS_GROUPBOX, rect, this, (UINT)-1);
...
{
	MessageBox(_T("This feature is currently unimplemented. Sorry!"), _T("Error"), MB_ICONINFORMATION | MB_OK);
}
...
void CMainWindow::OnSelChange()
{
	...
	else
	{
		...
		m_fontSample.CreateFont(-m_cyChar * 2, 0, 0, 0, FW_NORMAL, 0, 0, 0, DEFAULT_CHARSET, OUT_CHARACTER_PRECIS, CLIP_CHARACTER_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, strFaceName);
...
int CALLBACK CMainWindow::EnumFontFamProc(ENUMLOGFONT* lpelf, NEWTEXTMETRIC* lpnmt, int nFontType, LPARAM lParam)
{
