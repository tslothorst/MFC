...
BEGIN_MESSAGE_MAP(CStyleBar, CToolBar)
	ON_WM_CREATE()
	ON_CBN_SELENDOK(IDC_FONTNAMES, &CStyleBar::OnSelectFont)
	ON_CBN_SELENDOK(IDC_FONTSIZES, &CStyleBar::OnSelectFont)
	ON_CBN_CLOSEUP(IDC_FONTNAMES, &CStyleBar::OnCloseUp)
	ON_CBN_CLOSEUP(IDC_FONTSIZES, &CStyleBar::OnCloseUp)
END_MESSAGE_MAP()
...
void CStyleBar::OnUpdateCmdUI(CFrameWnd* pTarget, BOOL bDisableIfNoHandler)
{
	...
	CMyWordView* pView = (CMyWordView*)((CFrameWnd*)AfxGetMainWnd())->GetActiveView();
	...
	int nSizeFromComboBox = _tstoi(szSize);
...
int CStyleBar::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_wndFontNames.Create(WS_CHILD | WS_VISIBLE | WS_VSCROLL | CBS_DROPDOWNLIST | CBS_SORT, rect, this, IDC_FONTNAMES))
	{
		return -1;
	}
	...
	if (!m_wndFontSizes.Create(WS_CHILD | WS_VISIBLE | WS_VSCROLL | CBS_DROPDOWNLIST, rect, this, IDC_FONTSIZES))
	{
		return -1;
	}
...
void CStyleBar::OnSelectSize()
{
	...
	int nSize = _tstoi(szSize) * 20; // Need twips
...
{
	::EnumFontFamilies(pDC->m_hDC, NULL, (FONTENUMPROC)EnumFontNameProc, (LPARAM)this);
}

int CALLBACK CStyleBar::EnumFontNameProc(ENUMLOGFONT* lpelf, NEWTEXTMETRIC* lpntm, int nFontType, LPARAM lParam)
{
